...
void *thread_put(void *arg) {
    ...
    //printf("%p\n", delete(global_map, MAP_KEY(insert -> key_ptr, sizeof(int))).key.key_base);
    //printf("%p\n", get(global_map, MAP_KEY(insert -> key_ptr, sizeof(int))).val_base);
...
Test(map_suite, 02_multithreaded, .timeout = 3, .init = map_init, .fini = map_fini) {
    ...
    for(int index = 0; index < NUM_THREADS; index++) {
        ...
        *val_ptr = index * 2;
        ...
        insert = malloc(sizeof(map_insert_t));
    ...
    for (int i = 0; i < NUM_THREADS; i++){
        //printf("%p\n", global_map -> nodes[i].val.val_base);
    }
